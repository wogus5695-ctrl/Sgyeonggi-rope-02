...
function getTaskContent(task, regionTask, region) {
  ...
  switch (task) {
    ...
    case '옥상방수':
      return {
        ogImage: 'og-waterproof.jpg',
        hero: `${regionTask}는 기존 방수층의 들뜸, 균열, 배수 불량, 우레탄 노후 상태를 먼저 확인해야 합니다. 단순 덧칠보다 바탕면 정리와 균열 보수 후 방수층을 형성하는 과정이 중요합니다.`,
        releakDesc: `건물 상부의 옥상 바닥면은 태양열과 기후 변화에 직접적으로 노출되므로, 제대로 된 하지 정리 없이 방수 코팅만 올리게 되면 방수막이 들뜨고 부풀어 올라 수명이 짧아집니다.`,
        reLeakBox: `콘크리트 바닥면의 미세한 크랙부터 배수 파이프 주변 조인트 틈새까지 물이 머무는 고질적인 정체 구역을 찾아 꼼꼼하게 기초 작업을 수행해야 합니다.`,
        processDesc: `옥상 방수층 부풀음 및 균열 부위 진단 → 기존 노후 방수재 탈거 및 바탕면 샌딩 → 크랙 충진 및 하도 프라이머 도포 → 우레탄 중도 및 상도 마감 순으로 시공합니다.`,
        faq1Q: `${r} 옥상방수는 기존 방수층 위에 바로 시공해도 되나요?`,
        faq1Answer: `기존 방수층의 들뜸, 균열, 오염 상태에 따라 달라집니다. 들뜬 부위를 제거하지 않고 바탕면 정리 없이 바로 덧칠하면 재누수 가능성이 높아질 수 있어 면처리가 필수적입니다.`,
        faq3Q: `옥상 바닥의 물고임 현상도 방수 수명에 영향을 주나요?`,
        faq3Answer: `네, 옥상 바닥의 수평(구배)이 맞지 않아 물이 고여 있으면 우레탄 방수층이 화학적으로 빠르게 경화 및 부식되어 박리되기 쉽습니다. 올케어는 방수 작업 전 물이 고이지 않고 배수구로 흐르도록 정밀하게 점검합니다.`,
    ...
    case '옥상누수':
      return {
        ogImage: 'og-waterproof.jpg',
        hero: `${regionTask}는 옥상 바닥 균열, 배수구 주변 틈, 방수층 노후화로 인해 발생하는 경우가 많습니다. 누수 위치와 실제 유입 경로가 다를 수 있어 현장 확인이 중요합니다.`,
        releakDesc: `상가, 빌라, 단독건물의 최상층 세대에서 비만 오면 천장이 변색되거나 젖는 증상은 옥상 조인트 균열이나 부식된 배수 파이프 틈새를 타고 빗물이 유입되어 흐르는 것입니다.`,
        reLeakBox: `비 오는 날의 침투 시간과 천장 젖음 증상을 다각도로 추적하여, 옥상 슬라브의 미세 균열 및 누수 원인 지점을 정확히 포착하는 정밀 진단이 솔루션의 핵심입니다.`,
        processDesc: `최상층 세대 내부 천장 젖음 및 물 흔적 조사 → 옥상 바닥면 및 조인트 크랙 탐지 → 누수 유입 구간 균열 충진 및 옥상 하부 집중 방수 처리 순으로 마감합니다.`,
        faq1Q: `천장에서 물이 새면 무조건 옥상 문제인가요?`,
        faq1Answer: `꼭 그렇지는 않습니다. 옥상 방수층, 배수구, 외벽, 창틀 주변 등 여러 유입 경로를 함께 확인해야 합니다. 올케어는 종합적인 물길 추적을 통해 실제 발원지를 탐지해 드립니다.`,
        faq3Q: `옥상 방수 공사를 새로 한 지 얼마 안 되었는데 누수가 생겼다면 무엇이 원인인가요?`,
        faq3Answer: `방수제 도포 전 바탕면의 균열을 완벽히 메우지 않았거나, 벽면와 바닥이 만나는 조인트 틈새 마감이 불량하여 틈새로 물이 차올라 흐르는 현상일 수 있습니다. 올케어의 정밀 조사를 통해 원인을 찾아야 합니다.`,
        contactTitle: `${regionTask} 원인 물길 진단`,
        contactDesc: `실내 천장의 얼룩 상태와 비 오는 날 누수량 변화를 알려주시면 건물 골조와 옥상 균열 특성을 다각도로 비교해 문제 지점을 꼼꼼히 짚어 안내해 드립니다.`
      };
    case '외벽방수':
      return {
        ogImage: 'og-wall.jpg',
        hero: `${regionTask}는 외벽 균열, 창틀 주변 틈, 실리콘 노후 부위까지 함께 확인해야 합니다. 로프 접근이 필요한 구간은 작업 가능 여부와 안전 조건을 먼저 점검해야 합니다.`,
        releakDesc: `외벽 마감재인 적벽돌, 콘크리트, 판넬 틈새 등은 자외선과 온도 변화에 따라 크랙이 벌어지고 빗물이 안쪽으로 스며들어 건물 내구성에 치명적인 손상을 줍니다.`,
        reLeakBox: `로프 고공 작업을 통해 접근해야 하는 고층 빌딩이나 아파트 외벽면은 흔들림 없는 고정 장치와 안전 조건을 사전 확인하고 방수 실링재 및 기능성 발수 코팅을 도포해야 효과가 큽니다.`,
        processDesc: `건물 외벽 마감재 및 외벽 손상 상태 진단 → 로프 안전 거치 위치 확보 및 균열 정리 → 고신축성 외벽 방수 실란트 충진 및 발수제 도포 마감 순으로 시공합니다.`,
        faq1Q: `외벽방수와 외벽보수는 다른 작업인가요?`,
        faq1Answer: `외벽보수는 균열이나 파손 부위를 물리적으로 보수하는 의미가 강하고, 외벽방수는 빗물 유입을 막기 위해 방수재 도포, 발수 코팅, 방수 실링 처리 등을 포괄적으로 시행하는 예방성 방수 조치를 의미합니다.`,
        faq3Q: `외벽 발수제 코팅은 장기적인 방수 유리에 필수적인가요?`,
        faq3Answer: `네, 적벽돌이나 메지(줄눈) 부위는 빗물을 잘 흡수하므로 고성능 외벽 투명 발수제를 주기적으로 도포해주시면 표면 장벽을 형성해 수분 침투를 막고 동파로 인한 외벽 균열을 미연에 방지합니다.`,
    ...
    case '외벽누수':
      return {
        ogImage: 'og-wall.jpg',
        hero: `${regionTask}는 외벽 균열, 창틀 실리콘 노후, 샷시 주변 틈으로 인해 발생하는 경우가 많습니다. 내부 물자국만 보고 판단하기보다 외부 유입 경로를 함께 확인해야 합니다.`,
        releakDesc: `비 오는 날 벽면 도배지가 넓게 변색되거나 누렇게 번지는 현상은 눈앞의 창문 틀뿐만 아니라 그보다 훨씬 높은 외벽 옹벽에 생긴 미세 실크랙에서 빗물이 안으로 들어가 모세관 현상으로 번지는 누수 지점일 확률이 큽니다.`,
        reLeakBox: `건물 틈새의 습기와 유입 경로를 겉보기 판정으로만 대처하면 엉뚱한 부위를 메우게 되어 하자가 반복되므로, 로프 정밀 안착 진단으로 크랙 시작점을 정확히 짚어내 해결해야 합니다.`,
        processDesc: `실내 벽 젖음 및 벽돌 백화 현상 조사 → 고공 외벽 균열 상태 및 창틀 경계면 검사 → 외벽 균열 보수제 충진 및 침투성 코팅 마감 순으로 진행합니다.`,
        ...
        faq3Q: `외벽에 백화 현상이 생기는 이유가 누수와 직접 관계가 있나요?`,
        faq3Answer: `네, 그렇습니다. 외벽 콘크리트나 벽돌 메지 사이로 지속해서 빗물이 유입되어 시멘트 내부의 석회 성분이 녹아 나와 굳어지는 현상이 백화입니다. 이미 물길이 트여 옥내 누수로 연결된 상태이므로 신속히 올케어 진단을 받아야 합니다.`,
        contactTitle: `${regionTask} 원인 탐색 정밀 진단`,
        contactDesc: `건물 외부 사진 and 빗물 유입 흔적을 알려주시면, 내부로 번지는 습기 발생지와 옹벽 크랙 분석을 거쳐 합리적인 보수 솔루션을 안내합니다.`
      };
    default:
      return {
...
function buildDynamicMeta(region, task, rawK) {
  ...
  switch (task) {
    ...
    case '옥상방수':
      ...
      h1Suffix = '방수층 노후·균열 점검';
      description = `${region} 옥상방수는 기존 방수층의 들뜸, 균열, 배수 불량, 우레탄 노후 상태를 먼저 확인해야 합니다. 단순 덧칠보다 바탕면 정리와 균열 보수 후 방수층을 형성하는 과정이 중요합니다.`;
      break;
    case '옥상누수':
      title = `${region} 옥상누수, 비 올 때 천장 누수 원인 확인 | 올케어`;
      h1Suffix = '천장 누수 원인 확인';
      description = `${region} 옥상누수는 옥상 바닥 균열, 배수구 주변 틈, 방수층 노후화로 인해 발생하는 경우가 많습니다. 누수 위치와 실제 유입 경로가 다를 수 있어 현장 확인이 중요합니다.`;
      break;
    case '외벽방수':
      title = `${region} 외벽방수, 외벽 균열·창틀 주변 방수 점검 | 올케어`;
      h1Suffix = '외벽 균열·창틀 주변 방수 점검';
      description = `${region} 외벽방수는 외벽 균열, 창틀 주변 틈, 실리콘 노후 부위까지 함께 확인해야 합니다. 로프 접근이 필요한 구간은 작업 가능 여부와 안전 조건을 먼저 점검해야 합니다.`;
      break;
    case '외벽누수':
      title = `${region} 외벽누수, 외벽 크랙·창틀 유입 경로 진단 | 올케어`;
      h1Suffix = '외벽 크랙·창틀 유입 경로 진단';
      description = `${region} 외벽누수는 외벽 균열, 창틀 실리콘 노후, 샷시 주변 틈으로 인해 발생하는 경우가 많습니다. 내부 물자국만 보고 판단하기보다 외부 유입 경로를 함께 확인해야 합니다.`;
      break;
    default:
